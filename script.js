...
AllLessons.innerHTML = `Agriculture Tutorials`
...
function loadMusic(indexNumb){
   mainVideo.src = `video/${allVideos[indexNumb - 1].src}.mp4`;
...
for(let i = 0; i < allVideos.length; i++){
   let liTag = `<li li-index="${i + 1}">
      ...
      <video class="${allVideos[i].id}" src="video/${allVideos[i].src}.mp4" style="display: none;" title="${allVideos[i].name}"></video>
